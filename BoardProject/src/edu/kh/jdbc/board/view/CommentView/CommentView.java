package edu.kh.jdbc.board.view.CommentView;

import java.util.InputMismatchException;
import java.util.Scanner;

import edu.kh.jdbc.board.model.dto.Comment.Comment;
import edu.kh.jdbc.board.model.service.CommentService.CommentService;
...
private void deleteComment(int boardNo) {
	...
	try {
		...
		int check = service.deleteCommentNo(commentNo, boardNo, Session.loginMember.getMemberNO());
		...
		while(true) {
			...
			char check2 = Character.toUpperCase(sc.next().charAt(0));
...
private void updateComment(int boardNo) {
	...
	try {
		...
		// 입력한 댓글 번호가 해당 게시글에 대해 로그인이 한 회원이 작성한 댓글이 맞는지 확인
...
private void insertComment(int boardNo) {
  ...
  System.out.println("[댓글 입력] <!wq 입력시 종료>");
